...
def get_videos(ws, prompt, input_type="image", person_count="single"):
    ...
    def _collect_video_paths(node_output):
        found = []
        for _key, _val in node_output.items():
            if not isinstance(_val, list):
                continue
            for _item in _val:
                if not isinstance(_item, dict):
                    continue
                _p = _item.get("fullpath")
                if not _p and _item.get("filename"):
                    _p = os.path.join("/ComfyUI/output", _item.get("subfolder", ""), _item["filename"])
                if _p and str(_p).lower().endswith((".mp4", ".webm", ".mov", ".mkv", ".gif")):
                    found.append(_p)
        return found

    for node_id in history["outputs"]:
        node_output = history["outputs"][node_id]
        videos_output = _collect_video_paths(node_output)
        if videos_output:
            logger.info(f"[PATCH] 노드 {node_id}에서 비디오 {len(videos_output)}개 발견: {videos_output}")
    ...
    if not any(output_videos.values()):
        logger.warning("[PATCH] 히스토리에서 비디오 없음 -> /ComfyUI/output 직접 스캔")
        import glob as _glob
        _cands = []
        for _ext in ("mp4", "webm", "mov", "mkv"):
            _cands += _glob.glob(f"/ComfyUI/output/**/*.{_ext}", recursive=True)
        _now = time.time()
        _recent = [(os.path.getmtime(p), p) for p in _cands if _now - os.path.getmtime(p) < 3600]
        _recent.sort(reverse=True)
        if _recent:
            logger.info(f"[PATCH] 스캔 발견 {len(_recent)}개, 최신: {_recent[0][1]}")
            output_videos["__scan__"] = [p for _, p in _recent]
...
def handler(job):
    ...
    _candidates = [p for _paths in videos.values() for p in _paths if p and os.path.exists(p)]
    if _candidates:
        output_video_path = max(_candidates, key=os.path.getsize)
        logger.info(f"[PATCH] 후보 {len(_candidates)}개 중 최대 크기 선택: {output_video_path} ({os.path.getsize(output_video_path)} bytes)")
